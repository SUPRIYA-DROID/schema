...
mongoose.connect('mongodb://127.0.0.1:27017/blogDB')
  .then(async () => {
    ...
    // ✅ Delete the first user (subbu) automatically on startup
    const firstUser = await User.findOne().sort({ createdAt: 1 }); // Get the oldest user
    if (firstUser && firstUser.username !== 'subbu123') {
...
app.get('/add-user', async (req, res) => {
  try {
    const existingUser = await User.findOne({ username: 'subbu123' });
    ...
    const newUser = new User({
      ...
      roles: ['admin'],
      profile: { firstName: 'subbu', lastName: 'h', age: 25 }
    });
